...
def train(
    ...
    if dataset == "MNIST":
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[round(i * num_batch) for i in dec_lr], gamma=0.1
        )
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[round(i * num_batch) for i in dec_lr], gamma=0.2
        )
    else:
    ...
    # sequence gives the training batches
    if sequence is None:
            ...
            if resume:
                try:
                    ...
                    while os.path.exists(
                        os.path.join(save_dir, f"model_step_{ind + 1}")
                    ):
                        ind = ind + 1
                    if ind >= 0:
